...
const createVehicleDocument = async (payload, reqUserId) => {
  ...
  const uploadedBy = payload.uploadedBy || reqUserId || null;
  if (uploadedBy) {
  ...
  const document = await VehicleDocument.create({
    ...
    documentType: (payload.documentType || payload.type || "").trim(),
...
const updateVehicleDocument = async (id, payload) => {
  ...
  if (payload.notes !== undefined) document.notes = payload.notes.trim();
