...
const listAssetDocuments = asyncHandler(async (req, res) => {
  console.info(`[asset-documents] GET query ${JSON.stringify(req.query)}`);
...
const createAssetDocument = asyncHandler(async (req, res) => {
  console.info(`[asset-documents] POST body ${JSON.stringify(req.body)}`);
  const document = await assetDocumentService.createAssetDocument(req.body, req.user?._id || null);
  res.status(201).json({
